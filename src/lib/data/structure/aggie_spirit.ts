    ...
    queryFn: async () => {
      ...
      return apiBaseData.routes.map((baseRoute): Route => {
        ...
        const directions: Direction[] = baseRoute.directionList.map((dir) => {
          ...
          const filteredPoints = patternPathForDirection.patternPoints.filter((pt) => !!pt.stop);
      ...
      return apiBuses.vehiclesByDirections.flatMap((vehicleDirection): Bus[] => {
        ...
        const directionBuses: Bus[] = vehicleDirection.vehicles.map((vehicle) => {
          return {
            ...
            capacity: Math.round((vehicle.passengersOnboard / vehicle.passengerCapacity) * 100),
  ...
  const query = useDependencyQuery<StopSchedule[]>({
    ...
    queryFn: async () => {
      ...
      return timetableEstimateData.routeStopSchedules.map((routeStop) => {
          ...
          (stopTime) =>
            ({
              ...
              estimatedTime: stopTime.estimatedDepartTimeUtc
                ...
                : null,
              scheduledTime: moment.utc(stopTime.scheduledDepartTimeUtc).local(),
        ...
        const route = apiRoutes.data?.find((r) => r.routeCode === routeStop.routeNumber)!;
